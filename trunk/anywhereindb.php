...
$conf_data = array(
					...
					'dbname'=> empty($_SESSION['dbname'])?'':$_SESSION['dbname'],
					// display 
					'show_summary'=> TRUE,
					'summary_number' => 20 // number of summary will be shown 
...
function db_connect ($server, $dbuser, $pass, $dbname) {
	...
	if (!$link) {
		...
		//$a = 'nnnnnnnnn';
	//	echo $a,$conf_data['db_error'], 'iiiii' ;
		return FALSE;
	}
...
?>
<html>
<title>AnyWhereInDB :: Comprehensive Search on MYSQL Database :: Search anything from the DB </title>
<style>
/*RR 10*/

.hg,.sql, #error, #notice{border-radius:10px;}
#wlg{position:absolute; right: 10; top: 5;}
.m{min-height: 400px; }
#h1,.mat{margin: 15px auto;}
#h1{width:350px;}
.mat{width:500px; text-align: right;}
#search_text{width: 400px;}
#search_text,input{box-shadow: 2px 2px 2px;}
h1{color:#233E99;font-size: 2em;font-weight:bold;}
h1,.title,.tag{text-shadow: #ccc 2px 2px 4px;text-align: center;}
.dn{display:none;}

#error, #notice{padding:10px;}
#error{background-color:pink;color:red;}
#notice{background-color:#00B000;}
...
.hg{background-color:yellow;padding:1px;}
.isql,.tag,.smr{font-size:small;}
.tag{margin-top:-25px;font-weight: bold;}
.sql{background-color:#cfc;padding:10px;font:small italic;}
.smr{color:#666;margin:10px;font-weight: bold;font-style:italic}
.me{margin-top: 10px;}
...
<div class=m>
	...
	<h1> AnyWhereInDB </h1>
	<div class=tag><span>Comprehensive Search on MYSQL Database</span></div>
	<div id="notice" class=dn > Notice: </div>
	<div id="error" class=dn> Errors: </div>
	<div id="wlg"> Searching DB: <span id="login_dbname">  </span> <a id=logout href="javascript:logout();">[Change Database] | Disconnect</a> </div>
	<div id="h1">
			...
			<form id="login" class=dn action="<?php echo $_SERVER['PHP_SELF'];?>" method="POST">
				<table>
						<tbody>
							...
							<tr>
								<td><label for=pass>Database Password </label></td>
...
(function(w){
    var dc = w.document, //lib
		...
		show_search = function () {
			id('login_dbname').innerHTML = conf_data.dbname;
			show(id('login'),'hide');
			show(id('wsearch'));
			show(id('wlg'));
		},
		...
		summary_count = 0,
		build_summary = function(text, i, j){
			...
			if(summary_count < conf_data.summary_number){
				//text = text.replace(/>/g,"&gt;").replace(/</g,"&lt;").replace(search_text_d, "<span class=hg>\$1</span>");
				//text = text.replace(search_text_d, "<span class=hg>\$1</span>");
				var patt=new RegExp(".{0,10}"+search_text+".{0,10}",'igm');
				
				text = text.match(patt);
				...
				if(text === null){
					...
					text = text[0].replace(/>/g,"&gt;").replace(/</g,"&lt;");
					summary_count += 1;
				}
				return ' ... ' + text+ ' ... ';
			}
		...
		populate_table = function(result) {
			...
			function single_table(tableno, tb_result) {
				...
				summary = el('div', 'smr','sm'+tableno),
				tb = el('table', 'tb tbi', 't'+tableno),
				tr,
				td,
				a = el('a', 'title'),
				i, j,
				...
				a.innerHTML = '<span id=ct'+ tableno +'>'+
					((conf_data.show_summary === true)?'+':'-')
					+'</span>'+ tables_to_search[tableno] +' &nbsp;',
			...
			//	console.log(tb_result[0]);
				for(i = 0; i<tb_result.length; i += 1) {
					...
					for(j = 0; j<tb_result[i].length; j += 1) {
						...
						if(summary_count < conf_data.summary_number && conf_data.show_summary === true) {
							summary.innerHTML += build_summary(tb_result[i][j], i, j);	
						}
						
						//console.log(tb_result[i][j]);
						tr.appendChild(td);
					}
					tb.appendChild(tr);
				}

				tb.style.display = (conf_data.show_summary === true)?'none':'block';
				summary.style.display = (conf_data.show_summary === true)?'block':'none';
		...
		toggle_table = function(table_no) {
			...
			toggle(id('sm'+table_no));
			w.location.hash = ('d'+ table_no);
//			console.log('hide table' + table_no);
		...
		load_information = function (rs) {
		 ...
		 if(rs.length === 0){
		 	error('Error! No Data has found!');
		 	return false;
		 }
		 //	console.log(rs, typeof rs, rs.length);
			data = JSON.parse(rs);
	...
	id('login').addEventListener('submit', function(e) {
		...
		ajax("GET", conf_data.url, logininfo, function (rs) {
			...
			if (rs.length !== 0) {
				...
				show_login();
				id('error').innerHTML = 'Error in Database Connecting.  Please try again. ';
				show(id('error'));
			}
	...
	id('search').addEventListener('submit', function(e) {
		e.preventDefault();
		//console.log(logininfo);
		id('pp_search').innerHTML = '<h4 id="re_text"></h4><p id="result"></p><div id="tbs"></div>';
